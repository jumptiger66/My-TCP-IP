...
net_err_t mblock_init (mblock_t * mblock, void * mem, int blk_size, int cnt, nlocker_type_t type){
    /*
     * 作用: 初始化数据包，将mem开始的内存区域划分成多个相同大小的内存块，然后用链表链接起来
     * param: mblock, 数据包结构
     * param: mem, 数组起始地址
     * param: blk_size, 内存块大小
     * param: cnt, 块数
     * param: nlocker_type_t, 该内存块需要的锁类型（一般互斥锁）
     */
